...
def load_subject_clean(n: int, data_dir=None) -> SubjectData:
    """
    load_subject(n), with config.BAD_CHANNELS[n] removed from ALL of
    train_ecog/test_ecog's channel columns BEFORE any downstream processing
    (channel selection, AR features, regression features all see a clean
    channel pool from the start). See config.BAD_CHANNELS docstring --
    these are reused directly from the Wiener team's own detection on the
    same physical data, not re-derived here.
    """
    sd = load_subject(n, data_dir=data_dir)
    bad = C.BAD_CHANNELS.get(n, [])
    if not bad:
        return sd
    keep = [c for c in range(sd.n_channels) if c not in bad]
    return SubjectData(
        subject=n,
        train_ecog=sd.train_ecog[:, keep],
        train_glove=sd.train_glove,
        test_ecog=sd.test_ecog[:, keep],
        test_glove=sd.test_glove,
    )
